...
void find_ide_cb(uint16_t bus, uint8_t device,
...
int find_ide_controller() {
    ide_found = 0;
    pci_foreach(find_ide_cb);
    return ide_found;
}


void ata_init_channels() {
    uint32_t bar0 = pci_read_bar(ide_dev.bus, ide_dev.device, ide_dev.function, 0);
    uint32_t bar1 = pci_read_bar(ide_dev.bus, ide_dev.device, ide_dev.function, 1);
    uint32_t bar2 = pci_read_bar(ide_dev.bus, ide_dev.device, ide_dev.function, 2);
    uint32_t bar3 = pci_read_bar(ide_dev.bus, ide_dev.device, ide_dev.function, 3);
    uint32_t bar4 = pci_read_bar(ide_dev.bus, ide_dev.device, ide_dev.function, 4);
